...
export default class PropertyTypeStore {
  ...
  @action getPropertiesByAvailable = (propertyTypeId: string) => {
    this.loadingInitial = true;
    try {
      const propertyType = this.propertyTypeRegistry.find(
        (propertyType: any) => propertyType.key === propertyTypeId
      );
  ...
  @action loadPropertyTypes = async () => {
    ...
    try {
      ...
      runInAction('error loading property types', () => {
        this.loadingInitial = false;
      });
      console.log(error);
    }
